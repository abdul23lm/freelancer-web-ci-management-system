...
class gaji_karyawan extends CI_Controller {
	...
	function tambah()
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$d['karyawan'] = $this->db->get("lmcp_karyawan");
	...
	function edit($id_param)
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$get = $this->db->get_where("lmcp_gaji_karyawan",$id)->row();
	...
	function simpan()
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			if($st=="tambah")
			{
				$this->db->insert("lmcp_gaji_karyawan",$dt);
				redirect("dashboard/gaji_karyawan");
			}
			else if($st=="edit")
			{
				$this->db->update("lmcp_gaji_karyawan",$dt,$id);
				redirect("dashboard/gaji_karyawan");
			}
		}
		else
	...
	function hapus($id_param)
	{
		...
		{
			$id['id_gaji_karyawan'] = $id_param;
			$get = $this->db->delete("lmcp_gaji_karyawan",$id);
			redirect("dashboard/gaji_karyawan");
		}
		else
		{
