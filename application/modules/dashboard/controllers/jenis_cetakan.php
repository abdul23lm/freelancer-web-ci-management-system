...
class jenis_cetakan extends CI_Controller {
	...
	function tambah()
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$d['satuan'] = $this->db->get("lmcp_jenis_satuan");
	...
	function edit($id_param)
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$get = $this->db->get_where("lmcp_jenis_desain",$id)->row();
	...
	function simpan()
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			if($st=="tambah")
			{
				$this->db->insert("lmcp_jenis_desain",$dt);
				redirect("dashboard/jenis_cetakan");
			}
			else if($st=="edit")
			{
				$this->db->update("lmcp_jenis_desain",$dt,$id);
				redirect("dashboard/jenis_cetakan");
			}
		}
		else
	...
	function hapus($id_param)
	{
		...
		{
			$id['kode_jenis_cetakan'] = $id_param;
			$get = $this->db->delete("lmcp_jenis_desain",$id);
			redirect("dashboard/jenis_cetakan");
		}
		else
		{
