...
class pembayaran extends CI_Controller {
	...
	function edit($id_param,$id_pemesanan)
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$get = $this->db->select("*")->join("lmcp_pemesanan","lmcp_pemesanan.kode_pemesanan=lmcp_pembayaran.kode_pemesanan")->get_where("lmcp_pembayaran",array("kode_pembayaran"=>$id_param))->row();
			...
			$get_head = $this->db->get_where("lmcp_pemesanan",array("kode_pemesanan"=>$get->kode_pemesanan))->row();
			...
			(select x.nama_cetakan, y.satuan,  x.harga, x.kode_jenis_cetakan from lmcp_jenis_desain x left join lmcp_jenis_satuan y on x.id_jenis_satuan=y.id_jenis_satuan) 
	...
	function cetak($id_param)
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$get = $this->db->get_where("lmcp_pembayaran",array("kode_pemesanan"=>$id_param))->row();
	...
	function simpan()
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$this->db->update("lmcp_pemesanan",$up,array("kode_pemesanan" => $id['kode_pemesanan']));
			
			$this->db->update("lmcp_pembayaran",$d_header,$id);
	...
	function hapus($id_param)
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$this->db->delete("lmcp_pembayaran",$id);
			redirect("dashboard/pembayaran");
		}
		else
		{
