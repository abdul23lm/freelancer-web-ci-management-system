...
class pemesanan extends CI_Controller {
	...
	function tambah()
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$d['pelanggan'] = $this->db->get("lmcp_pelanggan");
			$d['jenis_cetakan'] = $this->db->get("lmcp_jenis_desain");
	...
	function ambil_data_bahan()
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$q = $this->db->query("select * from lmcp_jenis_desain a left join lmcp_jenis_satuan b on a.id_jenis_satuan=b.id_jenis_satuan where a.kode_jenis_cetakan='".$kd."'");
			foreach($q->result() as $d)
			{
			?>
			<table cellpadding="0" cellspacing="0" border="0">
			<tr><td width="160">Kode Desain</td><td width="20">:</td><td><input type="text" value="<?php echo $d->kode_jenis_cetakan; ?>" class="input-read-only"
			 style="width:350px;" name="kode_jenis_cetakan" readonly="readonly" /></td></tr>
			<tr><td>Jenis Desain</td><td>:</td><td><input type="text" value="<?php echo $d->nama_cetakan; ?>" class="input-read-only" style="width:350px;" name=
	...
	function edit($id_param)
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$d['no_nota'] = $id_param;
			$get_head = $this->db->get_where("lmcp_pemesanan",array("kode_pemesanan"=>$id_param))->row();
			...
			$get_detail = $this->db->query("select a.kode_jenis_cetakan, a.jumlah, b.nama_cetakan, b.harga, b.satuan from lmcp_pemesanan_detail a left join 
			(select x.nama_cetakan, y.satuan, x.kode_jenis_cetakan, x.harga from lmcp_jenis_desain x left join lmcp_jenis_satuan y on x.id_jenis_satuan=y.id_jenis_satuan) 
	...
	function cetak($id_param)
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$cek = $this->db->get_where("lmcp_pembayaran",array("kode_pemesanan"=>$id_param))->num_rows();
			if($cek>0)
			{
			...
			$get_detail = $this->db->query("select a.kode_jenis_cetakan, a.jumlah, b.harga, b.nama_cetakan, b.satuan from lmcp_pemesanan_detail a left join 
			(select x.nama_cetakan, y.satuan, x.kode_jenis_cetakan, x.harga from lmcp_jenis_desain x left join lmcp_jenis_satuan y on x.id_jenis_satuan=y.id_jenis_satuan) 
			b on a.kode_jenis_cetakan=b.kode_jenis_cetakan where a.kode_pemesanan='".$id_param."'");
	...
	function simpan_pembayaran()
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			if($d_header['bayar']>=$jumlah_harga)
			{
				...
				$this->db->update("lmcp_pemesanan",$up,array("kode_pemesanan" => $d_header['kode_pemesanan']));
			}
			
			
			$this->db->insert("lmcp_pembayaran",$d_header);
			$this->session->unset_userdata('kode_pelanggan');
			$this->session->unset_userdata('tgl_pesan');
			$this->session->unset_userdata('tgl_selesai');
			$this->session->unset_userdata('jumlah_harga');
	...
	function simpan_update()
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$this->db->update("lmcp_pemesanan",$d_header,$id_up);
			$this->db->delete("lmcp_pemesanan_detail",$id_up);
			foreach($this->cart->contents() as $items)
			{
				$d_detail['kode_pemesanan'] = $temp;
				$d_detail['kode_jenis_cetakan'] = $items['id'];
	...
	function hapus($id_param)
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$this->db->delete("lmcp_pemesanan",$id);
			$this->db->delete("lmcp_pemesanan_detail",$id);
			redirect("dashboard/pemesanan");
		}
		else
		{
